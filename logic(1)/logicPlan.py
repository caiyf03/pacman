...
def sentence3() -> Expr:
    ...
    first1 = conjoin(PacmanAlive_0, ~PacmanKilled_0)
    first2 = conjoin(~PacmanAlive_0, PacmanBorn_0)
    first = PacmanAlive_1 % disjoin(first1, first2)
    second = ~conjoin(PacmanAlive_0, PacmanBorn_0)
    third = PacmanBorn_0 #???这不是和前面矛盾了吗？
    return conjoin(first, second, third)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    list_item = itertools.combinations(literals, 2)
    x = []
    for i in list_item:
        s = disjoin(~i[0], ~i[1])
        x.append(s)
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    ll = atLeastOne(literals)
    mm = atMostOne(literals)
    return conjoin(ll, mm)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    PSE = PropSymbolExpr(pacman_str, x, y, time = now)
    return (PSE % disjoin(possible_causes))
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # pacman can not be in wall
    for location in all_coords:
        not_in_wall = PropSymbolExpr(wall_str, location[0],location[1]) >> ~(PropSymbolExpr(pacman_str, location[0], location[1], time=t))
        pacphysics_sentences.append(not_in_wall)
    # Pacman is at exactly one of the squares at timestep t.
    position = []
    for location in non_outer_wall_coords:
        position.append(PropSymbolExpr(pacman_str,location[0], location[1], time=t))
    temp = exactlyOne(position)
    pacphysics_sentences.append(temp)
    # Pacman takes exactly one action at timestep t.
    direction = []
    for action in DIRECTIONS:
        direction.append(PropSymbolExpr(action, time=t))
    temp = exactlyOne(direction)
    pacphysics_sentences.append(temp)
    # Results of calling sensorModel(...), unless None.
    if sensorModel:
...
def positionLogicPlan(problem) -> List:
    ...
    while i < 50:
        print(i)
        goal = conjoin(PropSymbolExpr(pacman_str, xg, yg, time=i))
        location = []
        if i == 0:
            KB.append(PropSymbolExpr(pacman_str,x0, y0, time=0))

        for l in non_wall_coords:
            location.append(PropSymbolExpr(pacman_str,l[0], l[1], time=i))
        KB.append(exactlyOne(location))
        know_base = conjoin(KB)
        model1 = findModel(conjoin(goal,know_base))
        if model1:
            print("Q4 we get the way")
            return extractActionSequence(model1,actions)
        else:
            temp = []
            for action in actions:
                temp.append(PropSymbolExpr(action, time=i))
            KB.append(exactlyOne(temp))
            know_base = conjoin(KB)
        i = i+1
        for location in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(location[0],location[1], i, walls_grid))
        know_base = conjoin(KB)
...
def foodLogicPlan(problem) -> List:
    ...
    for location in food:
        KB.append(PropSymbolExpr(food_str, location[0], location[1], time=0))
    i=0
    while i < 50:
        print(i)
        if i == 0:
        ...
        i = i+1
        for location in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(location[0], location[1], i, walls))
        i =i-1
        # get the food location
        remain_food = []
        for location in food:
            remain_food.append(PropSymbolExpr(food_str, location[0], location[1], time=i))

        for location in food:
            # can we eat a food
            pacman_is_there = PropSymbolExpr(pacman_str,location[0], location[1], time=i)
            food_is_there = PropSymbolExpr(food_str, location[0], location[1], time=i)
            now_food_is_there = PropSymbolExpr(food_str, location[0], location[1], time=i+1)
            condition1 = ~food_is_there
            condition2 = conjoin(pacman_is_there, food_is_there, ~now_food_is_there)
            condition3 = conjoin(~pacman_is_there, food_is_there, now_food_is_there)
            condition = disjoin(condition1, condition2, condition3)
            KB.append(condition)
        know_base = conjoin(KB)
        model1 = findModel(conjoin(~disjoin(remain_food),know_base))
        print("a")
        if model1:
            print("finish")
            return extractActionSequence(model1,actions)
...
def localization(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # pacman can  not e in the wall
    for location in all_coords:
        if location in walls_list:
            KB.append(PropSymbolExpr(wall_str,location[0], location[1]))
        else:
            KB.append(~PropSymbolExpr(wall_str,location[0], location[1]))

    for i in range(agent.num_timesteps):
        # add pacphysics:
        pac=pacphysicsAxioms(i,all_coords, non_outer_wall_coords, walls_grid, sensorAxioms,allLegalSuccessorAxioms)
        KB.append(pac)
        #add actiopn:
        ac=PropSymbolExpr(agent.actions[i], time=i)
        KB.append(ac)
        #add percept
        fo = fourBitPerceptRules(i, agent.getPercepts())
        KB.append(fo)
        # where pacman can be
        possible_locations=[]
        Knowledge_base = conjoin(KB)
        for location in non_outer_wall_coords:
            where = PropSymbolExpr(pacman_str, location[0], location[1], time=i)
            if findModel(conjoin(Knowledge_base, where)):
                coordinate=(location[0],location[1])
                possible_locations.append(coordinate)
            if entails( Knowledge_base, where):
                KB.append(where)
                Knowledge_base = conjoin(KB)
            elif entails( Knowledge_base, ~where):
                KB.append(~where)
                Knowledge_base = conjoin(KB)
            else:
                print("Q6 should be here?")
   ...
   # for t in range(agent.num_timesteps):
        ...
        #yield possible_locations
#______________________________________________________________________________
def is_a_wall(map,x,y):
    if map[x][y] == 1:
        print("we find it a wall")
    elif map[x][y] == 0:
        print("no wall")
    else:
        print("Q8 we still not know whether a wall exist but this should not happen")
# QUESTION 7
...
def mapping(problem, agent) -> Generator:
    ...
    is_a_wall(known_map,pac_x_0,pac_y_0)
    ...
    #yield known_map
    for i in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(i, all_coords, non_outer_wall_coords,known_map,sensorAxioms, allLegalSuccessorAxioms))
        # add actiopn:
        ac = PropSymbolExpr(agent.actions[i], time=i)
        KB.append(ac)
        # add percept
        fo = fourBitPerceptRules(i, agent.getPercepts())
        KB.append(fo)
        known_base = conjoin(KB)
        for location in non_outer_wall_coords:
            wall = logic.PropSymbolExpr(wall_str, location[0], location[1])
            if entails(known_base, ~wall):
                KB.append(~wall)
                known_map[location[0]][location[1]] = 0
                known_base = conjoin(KB)
            elif entails(known_base, wall):
                KB.append(wall)
                known_map[location[0]][location[1]] = 1
                known_base = conjoin(KB)
            else:
                print("Q7, add provably wall -- we still not know whether there is a wall")
                known_map[location[0]][location[1]] = -1
        agent.moveToNextState(agent.actions[i])
        yield known_map
...
def slam(problem, agent) -> Generator:
    ...
    if known_map[pac_x_0][pac_y_0] == 1:
        return
    elif known_map[pac_x_0][pac_y_0] == -1:
        print("Q8 we still not know whether a wall exist but this should not happen")
    ...
    KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
    for i in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(i, all_coords, non_outer_wall_coords, known_map, SLAMSensorAxioms, SLAMSuccessorAxioms))
        KB.append(logic.PropSymbolExpr(agent.actions[i], time=i))
        KB.append(numAdjWallsPerceptRules(i, agent.getPercepts()))
        pacman_loactions = []
        for location in non_outer_wall_coords:
            #wall locations , pacman location
            wall = PropSymbolExpr(wall_str, location[0], location[1])
            known_base = conjoin(KB)
            if entails(known_base, wall):
                KB.append(wall)
                known_map[location[0]][location[1]] = 1
                ...
                print("Q8, add provably wall -- we still not know whether there is a wall")
                known_map[location[0]][location[1]] = -1

            known_base = conjoin(KB)
            pacman = PropSymbolExpr(pacman_str,location[0], location[1], time=i)
            if findModel(conjoin(known_base, pacman)):
                pacman_loactions.append((location[0], location[1]))
            elif entails(known_base, pacman):
                KB.append(pacman)
            else:
                KB.append(~pacman)
        agent.moveToNextState(agent.actions[i])
    ...
    #for t in range(agent.num_timesteps):
        "*** END YOUR CODE HERE ***"
        yield known_map, pacman_loactions
