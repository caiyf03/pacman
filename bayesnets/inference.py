...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: BayesNet, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # the evidence
        evidence_list = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for i in eliminationOrder:
            # join the eliminated variables
            # evidence_list = (joinFactorsByVariable(evidence_list, i))[0]
            # joined_factor = (joinFactorsByVariable(evidence_list, i))[1]
            evidence_list, joined_factor = (joinFactorsByVariable(evidence_list, i))
            # print(evidence_list)
            # If a factor that you are about to eliminate a variable from has
            #         only one unconditioned variable, you should not eliminate it
            #         and instead just discard the factor.
            if len(joined_factor.unconditionedVariables()) == 1:
                # we will not append in this situation
                print("only one unconditioned variable")
            else:
                eliminated_factor = eliminate(joined_factor, i)
                # this is a list not set
                print(eliminated_factor)
                evidence_list.append(eliminated_factor)
        # join and normalize
        return normalize(joinFactors(evidence_list))
        "*** END YOUR CODE HERE ***"
