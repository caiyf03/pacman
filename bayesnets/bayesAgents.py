...
def constructBayesNet(gameState: GameState):
    ...
    "*** YOUR CODE HERE ***"
    # populate edge of x,y of the house
    # name all position and house variables using the constants(X_POS_VAR, FOOD_HOUSE_VAR, etc.) at the top of this file.
    # HOUSE_VARS [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
    ...
    # observed edges and give the domain
    for i in obsVars:
        edges.append((FOOD_HOUSE_VAR, i))
        edges.append((GHOST_HOUSE_VAR, i))
        variableDomainsDict[i] = [BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL]
...
def getMostLikelyFoodHousePosition(evidence: Dict, bayesNet: BayesNet, eliminationOrder: List[str]):
    ...
    "*** YOUR CODE HERE ***"
    # First, call the variable elimination method you just implemented to obtain
    #     p(FoodHouse | everything else).
    final_probability = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    # inspect the resulting probability
    #     distribution to find the most probable location of the food house. Return this.
    max_num = 0
    choosen = 0
    tt = []
    for i in final_probability.getAllPossibleAssignmentDicts():
        tempt = final_probability.getProbability(i)
        tt.append(tempt)
        if tempt >= max_num:
            max_num = tempt
            choosen = i
